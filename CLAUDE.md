...
#### Two Different Things Are Called "Unity Skills"

They are unrelated and the collision has already cost time. Say which one you mean:
...
2. **Unity Skills (`com.besty.unity-skills`)** — an unofficial third-party *Editor package* exposing REST modules on `http://localhost:8090`. Local-only, never committed, and **not currently installed**.
...
- **`com.unity.ai.assistant` (`2.13.0-pre.2`) IS in `Packages/manifest.json`**, so the deprecated package is one we ship. But its MCP server is **not** one of the editor bridges this project uses, so nothing in the current workflow breaks. Do not rip the package out on the strength of this announcement alone.
...
- Before using Unity Skills, read its package `SKILL.md` and the relevant module `SKILL.md` from the installed package. (This line used to name `/Users/ericslutz/.agents/skills/unity-skills/…`; that path does not exist and `com.besty.unity-skills` is not installed on this machine as of 2026-08-24, so the instruction was unfollowable as written.) Honor `currentMode`, approval grants, and forbidden-skill behavior. Do not add skills to the Allowlist unless Eric explicitly asks.
