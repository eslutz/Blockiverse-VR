...
- On Quest, launch splash/loading, menu, modal, and gameplay HUD surfaces may be backed by XR Quad composition layers for sharp text and artwork while preserving the same routing and ray interaction model. Routed game menus must share one generated Quad composition surface. Controller rays stay on the normal main-camera render path; menu-local hover feedback over the compositor surface comes from the generated composition menu cursor rather than a projection-layer ray workaround.
