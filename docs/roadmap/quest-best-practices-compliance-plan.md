...
| Keep controller/ray visuals on the main-camera path and add menu-local compositor hover feedback. | Implemented, verify on Quest. | `BlockiverseProjectBootstrapper` removes stale projection-layer ray rigs, leaves controllers/rays on the normal render path, culls composition source UI from the main camera, and generates a non-raycast composition menu cursor for menu-local UI hits. | `CompositionLayerUiEditModeTests.ControllerRayVisualsStayOnMainCameraPathWithCompositionMenuCursor`; native Quest pass must confirm rays stay stable/inline and the cursor tracks menu hits. |
...
| Adopt a measured Quest rendering baseline. | Implemented, verify on Quest. | Android URP asset uses Vulkan-side mobile defaults: depth texture off, opaque texture off, HDR off, 4x MSAA (Meta-recommended level), render scale 1.0, adaptive performance on. Shadows and additional lights are budgeted rather than disabled: main-light shadows on at 1024 with a single cascade over 30 m, additional lights per-pixel capped at 4 per object with shadow support on at 1024, hard shadows only. Disabling them outright made night unnavigable and made placed light blocks emit nothing. | `BlockiverseBootstrapEditModeTests.AndroidUrpAssetUsesQuestMobileRenderDefaults` and `.AndroidUrpAssetEnablesQuestBudgetedShadowsAndAdditionalLights`; Quest profiling must prove refresh stability on representative content, particularly the added ShadowCaster pass over loaded chunks. |
...
| Replace negative composition-layer tests with positive architectural tests. | Implemented. | Tests now assert the shared menu composition layer, absence of projection-layer ray rendering, the menu cursor, world-space exceptions, and Android composition splash path. | `CompositionLayerUiEditModeTests` and `QuestBestPracticesGovernanceEditModeTests`. |
