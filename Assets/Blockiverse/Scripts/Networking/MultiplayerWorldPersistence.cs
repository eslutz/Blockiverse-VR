...
namespace Blockiverse.Networking
{
    ...
    public sealed class MultiplayerWorldPersistence : MonoBehaviour
    {
        ...
        [SerializeField] MonoBehaviour worldManagerObject;
        [SerializeField] MultiplayerChunkAuthoritySync chunkAuthoritySync;
        [SerializeField] MultiplayerSurvivalSync survivalSync;
        [SerializeField] MonoBehaviour vitalsRuntimeObject;
        ...
        ISurvivalVitalsContext vitalsRuntime;
            ...
            string targetWorldName = null)
        {
            ...
            worldManagerObject = targetWorldManager as MonoBehaviour;
        ...
        bool RestoreSavedWorldBeforeHostStart(out string failureReason)
        {
            ...
            try
            {
                ...
                worldManager.SetGameMode(string.Equals(result.Data.GameMode, "creative", StringComparison.OrdinalIgnoreCase)
        ...
        void ResolveReferences()
        {
            ...
            {
                if (worldManagerObject != null && worldManagerObject is IMultiplayerWorldContext context)
                {
                    worldManager = context;
                }
                else
                {
                    worldManager = FindInterfaceObject<IMultiplayerWorldContext>();
                }
            }
            ...
            if (survivalSync == null)
                survivalSync = GetComponent<MultiplayerSurvivalSync>() ??
                    FindInterfaceObject<MultiplayerSurvivalSync>(FindObjectsInactive.Include);

            if (vitalsRuntime == null)
            {
                if (vitalsRuntimeObject != null && vitalsRuntimeObject is ISurvivalVitalsContext context)
                {
                    vitalsRuntime = context;
                }
                else
                {
                    vitalsRuntime = FindInterfaceObject<ISurvivalVitalsContext>();
                }
            }
        }
        ...
        static T FindInterfaceObject<T>(FindObjectsInactive inactive = FindObjectsInactive.Include) where T : class
        {
            var managers = FindObjectsByType<MonoBehaviour>(inactive, FindObjectsSortMode.None);
