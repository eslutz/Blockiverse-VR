    ...
    public sealed class M4ArtAssetValidationEditModeTests
    {
        ...
        public void AuthoredBlockAtlasesExistWithExpectedDimensionsAndImportSettings()
        {
            foreach (string path in ExpectedBlockAtlasPaths())
            {
                ...
                // Colour dilation into transparent texels. preserveCoverage rescales ALPHA per
                // mip; without this the RGB under those texels (pure black) is averaged in and
                // every blade grows a dark fringe that worsens with distance. Asserted per
                // texture set because only `enhanced` is regenerated by the main art script.
                ...
                // Android max texture size must be large enough to hold the larger dimension
                // without downscaling (which would misalign tiles), and no larger than the next
                // power of two above it to keep the Quest texture budget tight.
        ...
        [Test]
        public void CutoutFoliageDoesNotOccludeItsOwnInteriorButStillBlocksLight()
        {
            BlockRegistry registry = BlockRegistry.CreateDefault();
            BlockDefinition leaves = registry.Get(BlockRegistry.Leafmoss);
            BlockDefinition stone = registry.Get(BlockRegistry.Graystone);

            Assert.That(leaves.IsSolid, Is.True, "You must not fall through a canopy.");

            // The whole reason face occlusion and light occlusion are separate properties. Leaves
            // are the only block where they differ, so this pair IS the invariant:
            Assert.That(leaves.OccludesFaces, Is.False,
                "A canopy must hide no faces, or its alpha gaps reveal a hollow shell instead of interior leaves.");
            Assert.That(leaves.BlocksLight, Is.True,
                "A canopy must still block light, or forest floors stop being shaded (Pinewild §13.2) and torches shine through trees.");

            // A normal opaque block keeps both, inherited from IsSolid.
            Assert.That(stone.OccludesFaces, Is.True);
            Assert.That(stone.BlocksLight, Is.True);
        }

        [Test]
        public void EveryBlockInheritsBothOcclusionFlagsFromIsSolidUnlessItOptsOut()
        {
            BlockRegistry registry = BlockRegistry.CreateDefault();

            // Only leafmoss may differ today. If a second block ever needs to, that is a design
            // decision worth making deliberately rather than discovering as a lighting bug.
            foreach (BlockDefinition definition in registry.All)
            {
                if (definition.Id.Equals(BlockRegistry.Leafmoss))
                    continue;

                Assert.That(definition.OccludesFaces, Is.EqualTo(definition.IsSolid),
                    $"{definition.CanonicalId} diverges from IsSolid for face occlusion; only cutout leaves should.");
                Assert.That(definition.BlocksLight, Is.EqualTo(definition.IsSolid),
                    $"{definition.CanonicalId} diverges from IsSolid for light occlusion; only cutout leaves should.");
            }
        }
